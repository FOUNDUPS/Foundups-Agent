...
| 18 | REPORT_PATH_IGNORE_RULE_HANDLED_WITHOUT_SCOPE_WIDENING | YES | `.gitignore:324` (`docs/audits/*`) ignores the mandated report dir; `infrastructure/` lacks the negation pair other audit dirs have (`.gitignore:325-357`, 17 precedent pairs incl. `!docs/audits/architecture/**`). Committed via explicit `git add -f`, keeping the repo diff at exactly report + ModLog (a tracked file is unaffected by ignore rules). The 2-line negation pair (`!docs/audits/infrastructure/` + `/**`) is left as a follow-up for W10 to authorize - not silently added here |

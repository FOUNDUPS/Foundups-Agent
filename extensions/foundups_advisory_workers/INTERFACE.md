...
| `required_targets_in_model_context` | Required targets whose stable marker survives in the FINAL post-cut model-visible context (v0.3.35+; computed from the final string, NOT from fetch telemetry) | OBSERVED |
...
| `required_targets_context_missing` | Required target paths whose marker did NOT survive the 42K cut (v0.3.35+; `[]` when packing succeeded) | OBSERVED |
...
Exported helpers for contract tests: `isTargetReadPathDenied`, `resolveSafeRepoFile`, `readBoundedTargetSnippet`, `readBoundedTargetSnippets`, `buildTargetRecallContentSection`, `sanitizeTargetSnippetForRedaction`, `taskMentionsWsp97`, `buildWsp97ProtocolExcerpt`, `parseRequiredTargetPaths`, `isSelfFileLocation`, `requiredTargetMatchesLocation`, `formatHoloIndexScorecardLines`, `buildMustIncludeArgs`, `buildDirectReadContentSection`, `buildRequiredTargetProtectedSection`, `assembleFinalBoundedContext`, `computeRequiredTargetContextProof`.

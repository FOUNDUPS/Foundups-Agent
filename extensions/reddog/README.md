...
Version: 0.4.134
0.4.134 keeps every verified owner requery rooted in the original workspace/control checkout while the independently selected clean authority remains the proof target. This closes the candidate controller false-negative caused by recycling the authority as the workspace entry root; merged exact-main replay remains required. HoloIndex remains below A-grade and retrieval RSI remains blocked pending exact runtime closure, independent evaluation, promotion/canary/rollback, and candidate-bound outcome learning.
...
reserved for cleanup. At exact main `66526ae5`, three fresh-process calls passed
on attempt one in about 34 seconds each; an explicitly pre-warmed call passed in
10.3 seconds after a 25.5-second bootstrap. Cold availability is closed for that
commit without changing a timeout, but retrieval quality is not A-grade/RSI.
Process-local serialization and one cold child per query remain P1 throughput
debt; exact-commit evidence never authorizes a later HEAD.
...
to that backend path or add worker, repository, merge, or HoloIndex authority.

Version 0.4.58 lets a linked-worktree RedDog query use the authority checkout
for indexed repository bytes and the same repository's primary worktree for
vetted Python dependencies. It does not grant query-time maintenance or index
mutation authority.
...
backend manifest. It adds no extension-side authority. Production outcome
publication remains fail closed until an independently authenticated durable
verifier-authority source is implemented and supplied to the isolated signer.
...
that blocked or abstained. The retry budget remains one, a real framing or
WSP_15 challenge is still mandatory, and no OpenClaw/Hermes execution authority
...
no update, model-load, route, shell, or dispatch authority. Supplier output is
integrity-only: it reports observed match/drift details but remains overall
`NOT_READY` until signed source admission exists. Version 0.4.49
binds the read-only runtime compatibility advisory. Version 0.4.48 routes an exhausted,
...
query-time repair and editor-side indexing remain forbidden.
...
supersedes the quarantined local 0.4.42 Codex-intercept artifact, preserves
RedDog as the architect rather than replacing it with Codex, and packages the
current receipt-gated backend compatibility manifest.
...
logical roles and WSP operating discipline only; signed model-selection and
runtime-binding receipts continue to select the actual models.

Version 0.4.32 aligns cold HoloIndex owner startup supervision with the
existing semantic warmup budget while retaining the ordinary probe limit.

Version 0.4.31 lets the exact `start operations` control recover a failed or
stale HoloIndex semantic owner through one durable AgentDB task claimed by
OpenClaw. The worker reuses the canonical Holo maintenance handshake, proves
...
1,060-file backend manifest after signed-worker assignment, lease renewal, and
...
assigned verifier receives the canonical base/head receipt.
...
- Repository/module audit grounding (v0.4.10 compatibility): bounded prompts such as `Audit p.fMALL codebase` canonicalize punctuation/case aliases, retain the current generation-bound semantic owner path, and securely read structured-bundle candidates before treating implementation source plus independent test/contract coverage as sufficient. Missing, stale, or unreadable evidence triggers deterministic read-only discovery and merged/deduplicated selection. Fixed private/tool-state roots are never entered or selected. Final model context is capped at 12 files, 12KB per file, and 96KB total; candidate content scanning has a separate 4KB-per-file and 512KB-total budget. Missing post-pack proof returns `codebase_audit_evidence_incomplete` before a provider call.
- Schema-repair provenance: successful repair supplements missing output sections without replacing the primary Fusion `review_packet` or quorum truth; repair identity and call status remain under sibling `schema_repair_telemetry`.
...
- Defensive review contract: architect and critic prompts frame cybersecurity work around identification, prevention, and remediation while omitting unnecessary exploit detail. Empty or literal `None` critic responses are receipted as abstentions and cannot satisfy Fusion's challenging-critic quorum.
...
2. Run `Extensions: Install from VSIX...` and select the generated `reddog-0.4.134.vsix` (or current package version).

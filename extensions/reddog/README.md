...
Version 0.4.51 pins the opt-in OpenClaw/Hermes WSL availability adapter and its
generated backend manifest. The adapter is advisory-only, disabled by default,
and adds no worker authority. Version 0.4.50 binds the WRE-owned runtime
...
2. Run `Extensions: Install from VSIX...` and select the generated `reddog-0.4.51.vsix` (or current package version).

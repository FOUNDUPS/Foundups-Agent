...
Extension v0.4.6 invokes `scripts/reddog_holoindex_owner_query_once.py`, which reuses the authenticated localhost owner client and emits the canonical `holoindex_query_receipt.v1`. Semantic evidence is accepted only when the owner result and receipt agree on `CURRENT` freshness, semantic mode, clean repository HEAD, generation ID, freshness-receipt digest, and query-only behavior. The extension then emits `reddog_grounded_target_receipt.v1`, binding those proofs to the work focus and typed target universe. Quoted and fenced reference blocks remain context data and cannot become repo or external-research targets. The resident bridge, AgentDB cycle, OpenClaw swarm planner, task publisher, and read-only worker all revalidate that same receipt before model invocation. RedDog does not re-index HoloIndex during a reasoning run; WRE/CI owns maintenance.

...
includes(readme, 'Version: 0.4.75', 'README version mismatch');
...
assert.strictEqual(spinePreview.governed_work_order_candidate.red_dog_instance_id, 'foundups-agent-0.4.75', 'candidate must bind extension version');
...
includes(extensionSnippet.content, "const EXTENSION_VERSION = '0.4.75'", 'target snippet must include extension.js source');
...
includes(targetSection.text, "const EXTENSION_VERSION = '0.4.75'", 'target recall must include source snippet');
...
includes(boundedContext.text, "const EXTENSION_VERSION = '0.4.75'", 'bounded context must include source snippet');

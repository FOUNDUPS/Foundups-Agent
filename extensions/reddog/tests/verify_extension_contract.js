...
assert.strictEqual(pkg.version, '0.4.74', 'package version must be 0.4.74');
includes(extensionJs, "const EXTENSION_VERSION = '0.4.74'", 'extension build mismatch');
...
includes(readme, 'Version: 0.4.74', 'README version mismatch');
...
try {
  ...
  assert.strictEqual(hsfFallback.meta.holoindex_owner_attempts, 3, 'HSF-006: rejected owner retains bounded attempt telemetry');
...
assert.strictEqual(spinePreview.governed_work_order_candidate.red_dog_instance_id, 'foundups-agent-0.4.74', 'candidate must bind extension version');
...
includes(extensionSnippet.content, "const EXTENSION_VERSION = '0.4.74'", 'target snippet must include extension.js source');
...
includes(targetSection.text, "const EXTENSION_VERSION = '0.4.74'", 'target recall must include source snippet');
...
includes(boundedContext.text, "const EXTENSION_VERSION = '0.4.74'", 'bounded context must include source snippet');

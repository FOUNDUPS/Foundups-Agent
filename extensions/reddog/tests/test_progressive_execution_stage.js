...
assert.strictEqual(stage.runtimeBindingAllowsActionPlanning({
  model_binding_source: 'receipt_bound_runtime'
}), true);
assert.strictEqual(stage.runtimeBindingAllowsActionPlanning({
  model_binding_source: 'evaluation_config'
}), false);
assert.strictEqual(stage.runtimeBindingAllowsActionPlanning({
  model_binding_source: 'runtime_binding_rejected'
}), false);
...
const evaluationBounded = stage.project(stage.BOUNDED_EXECUTION,
  stage.runtimeBindingAllowsActionPlanning({ model_binding_source: 'evaluation_config' }));
assert.strictEqual(evaluationBounded.action_planning_ceiling_open, false);
...
assert(extension.includes('runtimeConsumptionGate.passed === true && actionRuntimeReady'));
assert(extension.includes('progressiveStage.receipt.action_planning_ceiling_open === true'));
assert(extension.includes('modelRuntimeBindingQuery.metadata(worker)'));
assert(extension.includes("Symbol('reddog.model_runtime_binding_egress')"));
...
const boundedContextBuild = askSource.indexOf('buildContextForRequest');
assert(earlyCompatibility >= 0 && boundedContextBuild > earlyCompatibility);
assert(extension.includes('plain: () => buildBoundedRepoContext(input.contextMode'));
assert(askSource.includes('const auditDegraded = !actionStageEnabled && compatibility.passed !== true'));
assert(askSource.includes('useEmpty: auditDegraded || localFastPath'));

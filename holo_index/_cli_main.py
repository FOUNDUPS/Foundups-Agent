...
def _generate_cli_catalog(scan_result, catalog_path: Path) -> None:
    ...
    lines = [
        ...
        f"**Total CLIs**: {scan_result.total_cli_entrypoints} | **WRE-Connected**: {len(scan_result.wre_connected)} | **Orphans**: {len(scan_result.orphans)} | **Registered SKILLz.md**: {scan_result.registered_skills}",
        "",
        "---",
        "",
...
def _generate_cli_rolodex_sqlite(scan_result, db_path: Path) -> None:
    ...
    cursor.execute("INSERT INTO rolodex_metadata VALUES (?, ?)", ("wre_connected_count", str(len(scan_result.wre_connected))))
    cursor.execute("INSERT INTO rolodex_metadata VALUES (?, ?)", ("registered_skillz_count", str(scan_result.registered_skills)))
...
# Removed noisy Unicode warnings - 0102 doesn't need internal system messages
    ...
    if index_cli:
        ...
        try:
            ...
            safe_print(f"[CLI] WRE-connected: {len(result.wre_connected)}, Orphans: {len(result.orphans)}, Registered SKILLz.md: {result.registered_skills}")
            indexing_awarded = True

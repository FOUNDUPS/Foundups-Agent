#!/usr/bin/env python3
"""
Refresh the OpenClaw PQN external research watchlist against official-source URLs.

This reuses the generic watchlist refresh helpers used by the grant tracker so
external benchmark repos can be monitored with the same change-detection path.
"""
...
WATCHLIST_PATH = (
    ...
    / "workspace"
    / "reports"
    / "pqn_external_research_watchlist.json"
)
STATUS_PATH = (
    REPO_ROOT
    / "modules"
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Refresh the OpenClaw PQN external research watchlist."
    )
    parser.add_argument(
        "--watchlist",
        type=Path,
    ...
    status_report["watch_type"] = "pqn_external_research"

...
ASSUMPTIONS_PATH = Path(__file__).resolve().parent / "assumptions" / "yumori_legacy_reconciled.json"
...
def debt_schedule(principal_jpy: float, annual_rate: float, term_years: int, horizon_years: int) -> Tuple[DebtYear, ...]:
    ...
    for year in range(1, horizon_years + 1):
        ...
        rows.append(DebtYear(year, beginning, principal_payment, interest, ending, total_payment))
        balance = ending
    return tuple(rows)
...
def _straight_line_depreciation(basis_jpy: float, useful_life_years: int, horizon_years: int) -> Tuple[float, ...]:
...
def build_model(assumptions: Mapping[str, Any] | None = None) -> ModelResult:
    ...
    if sum(tier_counts) != int(gpu["total"]):
    ...
    hardware_dep = _straight_line_depreciation(float(capital["compute_hardware_capex"]), int(depreciation_a["compute_hardware_life_years"]), horizon)
    infra_dep = _straight_line_depreciation(float(depreciation_a["infrastructure_basis_net_of_grants_jpy"]), int(depreciation_a["infrastructure_life_years"]), horizon)

    debt_a = a["debt"]
    compute_debt = debt_schedule(float(debt_a["compute"]["principal_jpy"]), float(debt_a["compute"]["annual_rate"]), int(debt_a["compute"]["term_years"]), horizon)
    green_debt = debt_schedule(float(debt_a["green"]["principal_jpy"]), float(debt_a["green"]["annual_rate"]), int(debt_a["green"]["term_years"]), horizon)
    ...
    for idx, year in enumerate(years):
        ...
        # Legacy reconciliation only: prior electricity row = 840 kW x 8,760 x utilization x tariff.
        ...
        sga = management[idx] + academic_marketing[idx] + insurance[idx] + property_lease[idx] + community[idx]
        ...
        debt_principal = compute_debt[idx].principal_repayment_jpy + green_debt[idx].principal_repayment_jpy
        fcfe = operating_cash_flow - maintenance_capex[idx] - debt_principal
        cumulative_fcfe += fcfe
        debt_service = compute_debt[idx].total_debt_service_jpy + green_debt[idx].total_debt_service_jpy
        dscr = ebitda / debt_service if debt_service else math.inf

        rows.append(ModelYear(year, u, tier_a, tier_b, tier_c, compute_revenue, thermal[idx], gross_revenue, electricity, cogs, gross_profit, sga, ebitda, depreciation, ebit, interest, ebt, tax, net_income, operating_cash_flow, maintenance_capex[idx], debt_principal, fcfe, cumulative_fcfe, dscr))
    ...
    result = ModelResult(
        ...
        equity_npv_jpy=npv(float(a["equity_discount_rate"]), equity_cash_flows),
        validation_messages=(),
    )
...
def validate_model(result: ModelResult, tolerance_jpy: float = 1.0) -> List[str]:
    ...
    sources = grants + float(capital["sponsor_equity"]) + float(capital["compute_debt"]) + float(capital["green_loan"])
    ...
    for row in result.years:
        if abs(row.tier_a_revenue_jpy + row.tier_b_revenue_jpy + row.tier_c_revenue_jpy - row.compute_revenue_jpy) > tolerance_jpy:
    ...
    if result.compute_debt[-1].ending_principal_jpy > tolerance_jpy:
    ...
    if len(result.years) > hardware_life:
        infra_annual = float(a["depreciation"]["infrastructure_basis_net_of_grants_jpy"]) / float(a["depreciation"]["infrastructure_life_years"])
        if abs(result.years[hardware_life].depreciation_jpy - infra_annual) > tolerance_jpy:

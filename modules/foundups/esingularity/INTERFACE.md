...
- [brand_context.json](brand_context.json) is authoritative for FoundUp identity on both sites, including eSingularity.ai, YUMORI.me's `.me`, movement aliases, JOIN copy, the Japanese-first guardian lockup, public domains and default locale. The website/report workflow separately owns JHR presentation labels. Supporting explanations must not replace either identity. English selection must render non-brand affected content in English; `html.lang` alone is not translation. This specifies required behavior and does not assert that legacy surfaces already comply.

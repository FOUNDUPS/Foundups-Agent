...
- The removed `frontend/content/yumori-presentation.ts` is historical only and must not be restored as a second truth source. Contracts and runtime code consume `yumori-vision.ts`.
- The visual deck uses the approved real-building and adaptive-reuse images referenced per slide by `yumori-vision.ts` and rendered with Next `Image`; structured text/evidence stays in HTML/TypeScript for accessibility, search, translation and RedDog grounding.
...
- Slide 01 is the real-building reuse vision. Slide 02 separates the reported demolition estimate and verified FY2018 use from reuse economics that remain under validation; unaudited revenue, profit and investor-return projections are excluded. Slides 03-10 cover D-K culture, onsen/wellness, AI rice field, COG DC heat reuse, 60 FoundUp projects/eSingularity Lab plus B1 gym/recovery, local problem-to-FoundUp, Fukui distributed-compute prototype, and the closing choice.

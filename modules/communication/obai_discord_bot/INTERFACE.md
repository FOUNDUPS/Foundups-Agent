...
class OBAIConfig:
    ...
    guild_id: int            # Guild for logging only (not enforced at runtime)
...
```python
{
    ...
    "guild_id": 412646632992014336,            # Configured guild (logging only)
...
| `on_message` | If bot is mentioned (not @everyone, not bot, not DM), reply with generic HELP_TEXT. Responds in **any guild** it's invited to — no guild filter. |
...
| Guild filtering | Designed in spec, not implemented |
...
|----------|----------|---------|-------|
| `OBAI_BOT` | YES | — | |
| `OBAI_GUILD_ID` | NO | `412646632992014336` | Logging only — not enforced |

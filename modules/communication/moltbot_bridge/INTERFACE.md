...
`ExternalSignerAuthoritativeUseLeaseIssuer` extends the existing socket-v2 and
E0 resolve-per-sign path. It binds one exact effect to the root-selected signer
profile and current generation, signs all four durable replay-store identities,
and releases only an opaque process-local capability. Signer and client must
observe the same E0 replay root. Cross-root replay, local-key substitution,
generation overrun, wall-clock rollback, socket-v1 downgrade, and effect
substitution fail closed. This interface remains inactive in production until
the external signer lifecycle and live-canary gates are provisioned.

...
def test_signed_revision_advances_monotonic_anchor(tmp_path: Path) -> None:
    ...
    created = _create(path, context, serialized)
    current = _store(path).load(created.conversation_id)["record"]
    next_turn = digest({"turn": "signed-second"})
    ...
    assert advanced.conversation_revision == 1
    persisted = _store(path).load(created.conversation_id)["record"]
    head = anchor.load()["heads"][created.conversation_id]
    assert head["conversation_revision"] == 1

...
class _ArtifactGenerator:
    def __init__(self, *, ok: bool = True, artifact_contents: dict[str, str] | None = None) -> None:
        self.ok = ok
...
def _artifact_generation_request(bundle: dict) -> dict[str, object]:
    ...
    return {
        ...
        "evidence_context": "The bounded pilot must materialize exactly one README fixture.",
        "holoindex_evidence": {
...
def test_dispatcher_generates_artifacts_before_bounded_worker_pilot(tmp_path: Path) -> None:
    bundle = _valid_bundle(tmp_path)
    work_order = _work_order_with_plan(bundle)
    chain_store = _seeded_store(bundle, **_binding_stage_overrides())
    generator = _ArtifactGenerator(artifact_contents={ARTIFACT: "# generated by RedDog\n"})
    handler = build_reddog_resident_queue_bounded_worker_pilot_stage_handler(
        chain_results_store=chain_store,
    ...
    assert (bundle["worktree"] / ARTIFACT).read_text(encoding="utf-8") == "# generated by RedDog\n"
    assert not (bundle["repo_root"] / ARTIFACT).exists()


def test_artifact_generation_requires_use_time_model_verifier(tmp_path: Path) -> None:
    bundle = _valid_bundle(tmp_path)
...
def test_rejected_artifact_generation_blocks_bounded_worker_pilot(tmp_path: Path) -> None:
    bundle = _valid_bundle(tmp_path)
    work_order = _work_order_with_plan(bundle)
    handler = build_reddog_resident_queue_bounded_worker_pilot_stage_handler(
        chain_results_store=_seeded_store(bundle, **_binding_stage_overrides()),
...
def test_artifact_generation_request_requires_explicit_generator(tmp_path: Path) -> None:
    bundle = _valid_bundle(tmp_path)
    work_order = _work_order_with_plan(bundle)
    handler = build_reddog_resident_queue_bounded_worker_pilot_stage_handler(
        chain_results_store=_seeded_store(bundle, **_binding_stage_overrides()),
...
def test_caller_supplied_artifact_authority_cannot_replace_durable_stage(
    ...
    assert result["decision"] == QUEUE_AUTHORIZED_BOUNDED_WORKER_PILOT_INVOKE_REJECT
    assert result["rejection_reasons"] == [FAIL_ARTIFACT_GENERATION_REQUEST_CONFLICT]
    assert generator.calls == []
    assert not (bundle["worktree"] / ARTIFACT).exists()

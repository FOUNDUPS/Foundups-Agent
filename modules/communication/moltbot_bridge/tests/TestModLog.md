...
- Proved root-selected signer identity, signer/client equality across all four E0 replay-store identities, selection-bounded expiry, and irreversible process-local expiry. Added the composed socket-v2 -> E0 -> resolve-per-sign -> external issuer -> WRE acceptance path.

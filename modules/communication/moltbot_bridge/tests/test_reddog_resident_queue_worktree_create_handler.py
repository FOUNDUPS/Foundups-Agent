"""Tests for REDDOG_RESIDENT_QUEUE_WORKTREE_CREATE_HANDLER_PHASE1."""
...
def _work_order(**overrides: object) -> dict[str, object]:
    payload: dict[str, object] = {
        ...
        "base_ref": "main",
        "task_summary": "Create isolated worktree from resident queue.",
        ...
        "holoindex_evidence": {
            "holoindex_query": "RedDog resident queue worktree create handler",
...
def _work_order_invocation_result() -> dict[str, object]:
    return {
...
def _executor_plan_result(repo_root: Path, **overrides: object) -> dict[str, object]:
    payload: dict[str, object] = {
...
def _execution_valve_result(**overrides: object) -> dict[str, object]:
    payload: dict[str, object] = {
...
def _seeded_store(repo_root: Path, **stage_overrides: object) -> InMemoryResidentQueueChainResultsStore:
...
def test_dispatcher_records_worktree_create_and_advances_to_bounded_worker_pilot(tmp_path: Path) -> None:
    ...
    stage = chain_store.load()["stage_results"][WORKTREE_CREATE_STAGE_KEY]
    ...
    assert stage["no_task_execution_performed"] is True
    assert stage["no_file_edit_performed"] is True
...
def test_missing_executor_plan_stage_rejects_direct_handler_call(tmp_path: Path) -> None:
    repo_root = _repo_root(tmp_path)
    handler = _handler(
        chain_store=_seeded_store(repo_root, **{EXECUTOR_PLAN_STAGE_KEY: {}}),
        repo_root=repo_root,
    )
    request = ResidentQueueStageDispatchRequest(
        stage_key=WORKTREE_CREATE_STAGE_KEY,
        next_action=NEXT_QUEUE_WORKTREE_CREATE_INVOKE,
        queue_item_id="queue-1",
        selected_slice="REDDOG_TEST_SLICE_PHASE1",
        plan_id="plan-1",
        accepted_stages=(),
...
def test_missing_execution_valve_stage_rejects_direct_handler_call(tmp_path: Path) -> None:
    repo_root = _repo_root(tmp_path)
    handler = _handler(
        chain_store=_seeded_store(repo_root, **{EXECUTION_VALVE_STAGE_KEY: {}}),
        repo_root=repo_root,
    )
    request = ResidentQueueStageDispatchRequest(
        stage_key=WORKTREE_CREATE_STAGE_KEY,
    ...
    assert result["decision"] == QUEUE_AUTHORIZED_WORKTREE_CREATE_INVOKE_REJECT
    assert FAIL_EXECUTION_VALVE_STAGE_MISSING in result["rejection_reasons"]
...
def test_wrong_stage_rejects_direct_handler_call(tmp_path: Path) -> None:
    repo_root = _repo_root(tmp_path)
    handler = _handler(chain_store=_seeded_store(repo_root), repo_root=repo_root)
    request = ResidentQueueStageDispatchRequest(
...
def test_wrong_next_action_rejects_direct_handler_call(tmp_path: Path) -> None:
    ...
    request = ResidentQueueStageDispatchRequest(
        stage_key=WORKTREE_CREATE_STAGE_KEY,
        next_action="RUN_QUEUE_AUTHORIZED_EXECUTION_VALVE_INVOKE",
        queue_item_id="queue-1",
        selected_slice="REDDOG_TEST_SLICE_PHASE1",
        plan_id="plan-1",
        accepted_stages=(),
...
def test_runner_failure_is_not_recorded_by_dispatcher(tmp_path: Path) -> None:
    repo_root = _repo_root(tmp_path)
    chain_store = _seeded_store(repo_root)
    ...
    assert WORKTREE_CREATE_STAGE_KEY not in chain_store.load()["stage_results"]

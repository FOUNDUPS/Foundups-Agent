...
from modules.communication.moltbot_bridge.tests.reddog_openclaw_claim_loop_test_support import (
    patch_fusion_artifact_generator as _patch_fusion_artifact_generator,
    pin_authority_epoch_receipts,
)
...
def test_profile_materializer_default_does_not_conflict_with_explicit_work_orders() -> (
    None
):
    env = {
...
def test_profile_signed_worker_queue_loop_runner_materializes_without_work_orders(
    tmp_path: Path,
) -> None:
    repo = _repo(tmp_path)
    ...
    result = build_reddog_signed_worker_queue_loop_runner_from_env(
    ...
    assert (
        result.runner.config.bootstrap_kwargs["work_order_materializer_mode"]
        == "authority_profile"
    )
...
def isolated_agent_db(tmp_path, monkeypatch):
    ...
    pin_authority_epoch_receipts(monkeypatch)
...
def test_main_openclaw_signed_worker_claim_loop_disabled_by_default() -> None:
    ...
    with patch(CLAIM_LOOP, side_effect=AssertionError("claim loop must not run")):
        with patch.dict("os.environ", {}, clear=True):
            assert (
                main.run_reddog_openclaw_signed_worker_claim_loop_preflight(REPO_ROOT)
                is True
            )
...
def test_main_openclaw_signed_worker_claim_loop_explicit_zero_overrides_profile() -> (
    None
):
    import main
...
def test_main_openclaw_signed_worker_claim_loop_rejects_invalid_max_claims_when_enforced() -> (
    None
):
    import main
...
def test_main_openclaw_signed_worker_claim_loop_exception_is_nonblocking_by_default() -> (
    None
):
    import main
...
def test_private_openclaw_claim_primitives_reject_without_control_lock(
    ...
    from modules.communication.moltbot_bridge.src import (
        openclaw_supervisor as supervisor,
    )
...
def test_main_resident_control_loop_consumes_signer_socket_started_by_runtime_cli(
    ...
    monkeypatch.setattr(
        worktree_pr_runner, "RealWorktreeRunner", _FakeProfileWorktreeRunner
    )
    repo = _repo(tmp_path)
    runtime_root = tmp_path / "resident-runtime"
    runtime_root.mkdir(parents=True)
    ...
    assert (
        signer_receipt["result"]["runtime_result"]["key_provider_receipt"][
            "profile_count"
        ]
        == 2
    )
    ...
    assert (
        stored["stage_results"]["bounded_worker_pilot"]["artifact_generation_result"][
            "accepted"
        ]
        is True
    )

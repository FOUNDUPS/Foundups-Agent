...
- Follow-up hardening accepts an exact all-pending batch only when durable
  ...
  superseded claim/use pair while retaining bounded result history whose full
  digests, previous-entry links, entry digests, and latest full receipt are
  revalidated before any runner call. Exact finalization lives in the bounded
  database execution store instead of extending the `AgentDB` monolith.

...
def route_foundup_job(job: Any) -> RouteEnvelope:
    ...
    try:
        ...
        if requested_action == "create_foundup":
            ...
            if not create_decision.ok or create_decision.request is None:
                return _make_blocked_envelope(
                    job_id=job_id,
                    tenant_id=tenant_id,
                    action=requested_action,
                    reason_code=RouteReasonCode.BLOCKED_INVALID_CREATE_BINDING,
                    reason_human=create_decision.error_human,
                    source_status=status_str,
                    foundup_id=getattr(job, "foundup_id", None),
                    policy_summary=policy_summary,
                )
            create_request = create_decision.request
        ...
        return RouteEnvelope(
            ...
            source_job_status=status_str,
            foundup_id=(
                create_request.foundup_id
                if create_request is not None
                else getattr(job, "foundup_id", None)
            ),
            creation_mode=(
                create_request.creation_mode if create_request is not None else None
            ),
            genesis_envelope_digest=(
                create_request.genesis_envelope_digest
                if create_request is not None
                else None
            ),
            scaffold_contract_digest=(
                create_request.scaffold_contract_digest
                if create_request is not None
                else None
            ),

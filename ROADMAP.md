...
R00 recovery is historical success at its recorded source/generation. R02 now includes a requirement/owner/test/gap map for R00–R25 and selected WSP 46/CORE and briefing corrections. The stale ledger's individual dispositions and separately bound review remain open. Do not mark all of R02 or G0 complete merely because the documentation is discoverable.
...
**012 scope correction, 2026-09-13:** RSI is a FoundUps system capability. Active FoundUps are not its experimental targets. YUMORI.me, eSingularity and every other active FoundUp remain excluded from this work's RSI tests, canaries, training/evaluation data, rollout and rollback experiments. They may eventually consume a proven system through separately scoped product adoption; their integration is not a prerequisite for the first RSI proof. Read-only repository architecture inspection is not permission to run a project or use its live state.

The first R15 target remains the internal test-registry-audit **work procedure**, with its independent correctness oracle fixed. Auto-post and the PQN researcher are additional existing system workflows to qualify in isolation; see the [candidate assessment and test boundaries](docs/operations/RSI_SWARM_DISPATCH.md#internal-rsi-validation-candidates). No candidate is approved for live execution by its inclusion in that table.

R20 uses a dedicated synthetic FoundUp fixture with disposable state, reusing existing fixtures where available. R22 demonstrates system work-state delivery in an isolated test consumer. Neither borrows an active project's account, deployment, traffic, data, browser session or database. R24/R25 qualification likewise uses synthetic agents/participants and test state. An active FoundUp's inclusion would require a separate explicit change of scope from 012; lane ownership or participant consent alone does not override this exclusion.
...
- **R20 — One FoundUp lifecycle vertical. P1.** Use a dedicated synthetic FoundUp fixture with a defined outcome and disposable state; reuse genesis, registry, scaffold, FAM, and work contracts. Active FoundUps are excluded. Validate idea/intake → admitted task → real bounded artifact → proof → independent verification → lifecycle update. Keep financial/chain actions outside this first proof. An externalized project requires its own exact repository evidence.

...
| Current RSI ticket path | PR1894 atomic verification is merged and retired with verified PR/main checks (164 fixed local/independent cases). [Actual AutoResearcher comparison](modules/infrastructure/wre_core/ROADMAP.md#actual-loop-comparison-qualification--2026-09-23) now has three fixed local/independent controls: identical candidate text can be accepted when only a shared cost changes. Next14/P1 freezes the consumed cost catalog per invocation; broader oracle/environment and domain/native admission remain open. No retained-RSI or AmIBot-build claim. |

...
| Current RSI ticket path | PR1887 program-call identity is merged and retired with exact PR/main checks. The evaluator dependency12/P2 qualification passes one real-evaluator case locally and independently: identical target bytes score differently when a synthetic cost changes; restoration reproduces baseline. Production is unchanged. Effective oracle/consumer/runtime authority remains open; the [backlog](docs/roadmaps/rsi_swarm_backlog.json) records evidence and fresh next work. AmIBot remains undispatched; Memory Horizon is separately owned. |

...
| Current RSI ticket path | PR1852 atomic pending initiation and PR1853 monitor/control contract are merged with main CI/CodeQL passed. Existing event-store sequence-collision retry15/P1 is locally repaired: nine fixed regressions pass, production-lock candidate replay remains unperformed after screening blocked its review. Exact independent/publication state is in the [backlog](docs/roadmaps/rsi_swarm_backlog.json). Next: qualify authoritative persistence/recovery/acknowledgment15/P1; role15/P1 remains open. Native trust anchors/no effect lease, settlement and retained RSI remain unproved. |

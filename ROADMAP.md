...
- LinkedIn preview qualification PR1869 is merged with verified PR/main checks. [Connection policy preview](modules/platform_integration/linkedin_agent/docs/LINKEDIN_REVIEW_WORKFLOW.md#rsi-connection-policy-preview--2026-09-22) now passes17 frozen cases locally and independently (baseline12fail/5pass), preserving bookkeeping and omitting request status. Same-file projection reduces inherited method/class/file size. Browser access, live authority and retained RSI remain separate. The canonical backlog records the fresh next ranking and exact publication evidence.

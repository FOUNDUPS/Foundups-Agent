...
| Current RSI ticket path | PR1888 evaluator dependency qualification is merged and retired with verified PR/main checks. The selected11/P2 use-time supplier repair suppresses generation acquisition after independently rejected bindings:15 fixed cases pass locally and independently, plus eight existing regressions. Seven native anchors/effect-lease admission remain unresolved. The [backlog](docs/roadmaps/rsi_swarm_backlog.json) records the next named persistent-verification ingress/entitlement decision. AmIBot remains undispatched; Memory Horizon is separately owned. |

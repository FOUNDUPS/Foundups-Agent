...
| Current RSI ticket path | PR #1877 canary qualification is closed. Core generation/readiness contract15/P1 passes11 frozen cases locally and independently: only three use-time generation reasons can clear; readiness stays blocked and no effect lease exists. Independent binding errors and unnecessary supplier acquisition remain characterized, not repaired. The [backlog](docs/roadmaps/rsi_swarm_backlog.json) records the next scored core action. AmIBot is registered but undispatched; Memory Horizon is separate PR #1822. Native WRE/RSI remains unproven. |

...
| Current RSI ticket path | PR1885 program-consumption qualification is merged and retired. The existing researcher now records per-attempted-call instruction identity;24 fixed local/independent cases and34 unchanged lifecycle regressions pass. Independent review caught and corrected a double-formatting mismatch. Exact independent replay/publication and re-scored next work are in the [backlog](docs/roadmaps/rsi_swarm_backlog.json). Local diagnostics do not admit native workers or certify retained learning. AmIBot remains undispatched; Memory Horizon is separately owned. |

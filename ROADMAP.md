...
| Current RSI ticket path | PR1901 instruction hygiene is merged, main-checked and retired. The existing manager-to-UI guard passes 36 fixed cases locally and independently (baseline 11 fail/25 pass); one source digest and two backend pins are refreshed. Exact publication/closure and next eligible 11/P2 test-runner documentation retention are in the [current ranked queue](docs/roadmaps/rsi_swarm_backlog.json). Native ticket admission and retained RSI remain open. |
